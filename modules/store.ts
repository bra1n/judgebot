...
export default class StoreLocator {
  ...
  static googleStaticMap =
    "https://maps.googleapis.com/maps/api/staticmap?scale=2&size=640x320&maptype=" +
    "roadmap&format=png&visual_refresh=true&key=" +
    process.env.GOOGLE_TOKEN;
  static googleStaticMapStandardMarkup = "&markers=size:mid|color:0x513dc2|";
  static storeLocator =
    "http://locator.wizards.com/Service/LocationService.svc/GetLocations";
  static eventsLocator =
    "http://locator.wizards.com/Service/LocationService.svc/GetLocationDetails";
  static wizardsStoreUrl =
    "http://locator.wizards.com/#brand=magic&a=location&massmarket=no&";
  static wizardsSearchUrl =
    "http://locator.wizards.com/#brand=magic&a=search&massmarket=no&p=";
  static storeLevels = [
    "Unknown",
    "Gateway",
    "Core",
  ...
  async fetchStores(longitude = -180, distance = 10) {
    try {
      ...
      const res = await fetch(StoreLocator.storeLocator, {
        method: "POST",
        body: JSON.stringify(
          this.generateStoreRequestBody(
            {
              location: {},
              bounds: {
                northeast: { lat: 85, lng: longitude + distance },
                southwest: { lat: -85, lng: longitude },
              },
            },
            5000
          )
      ...
      let count = 0;
      if (body.d.Results) {
        body.d.Results.forEach(({ Address, Organization, IsStore }: any) => {
      ...
      log.info(
        `retrieved ${count} stores between ${longitude} and ${
          longitude + distance
        } longitude`
      );
  ...
  async locateStores(location: any): Promise<InteractionReplyOptions> {
    ...
    try {
      const res = await fetch(
        StoreLocator.geocoder + encodeURIComponent(location)
      );
      ...
      return {
        embeds: [
          await this.generateErrorEmbed(
            err,
            "Couldn't retrieve location from Google."
          ),
        ],
      };
    }
    ...
    if (googleBody.results !== null && googleBody.results.length > 0) {
      if (!this.storeSearch || !this.stores.length) {
        ...
        const wizardsBody: any = await (
          await fetch(StoreLocator.storeLocator, {
            method: "POST",
            body: JSON.stringify(
              this.generateStoreRequestBody(
                googleBody.results[0].geometry,
                count
              )
            ),
          })
        ...
        try {
          return this.generateStoreEmbed(
            wizardsBody.d.Results,
          ...
          return {
            embeds: [
              await this.generateErrorEmbed(
                err,
                "Couldn't retrieve stores from Wizards."
              ),
            ],
          };
        }
  ...
  async generateStoreEmbed(
    ...
    stores.forEach(({ Address, Organization, IsStore }: any) => {
      ...
      fields.push({
        name: `${fields.length + 1}) ${Address.Name} (${Math.round(
          distance
        )}km)`,
      ...
      googleStaticMap.push(
        `${StoreLocator.googleStaticMapStandardMarkup}label:${fields.length}|${Address.Line1} ` +
    ...
    return {
      embeds: [
        {
          ...
          description: `:link: [Wizards Store Locator results](${
            StoreLocator.wizardsSearchUrl
          }${encodeURIComponent(googleResult.formatted_address)})`,
          image: fields.length
            ? {
                // only show map if there are actual stores
      ...
      files: [
        new AttachmentBuilder(res.body, {
          name: "location.png",
        }),
      ],
    };
    ...
    if (this.storeSearch) {
      ...
      if (stores.length) {
        ...
        try {
          ...
          return await this.generateErrorEmbed(
            err,
            "Error loading events from Wizards."
          );
        }
  ...
  generateEventsEmbed(
    ...
    const getDate = (event: any): Date =>
      new Date(parseFloat(event.StartDate.replace(/.*?(\d+)-.*/, "$1")));
    let footer = [];
    const fields: any = [];
    if (
      response &&
      response.d &&
      response.d.Result &&
      response.d.Result.EventsAtVenue
    ) {
      // merge events at venue with events outside of venue and sort by date
      let events: any = response.d.Result.EventsAtVenue.concat(
        response.d.Result.EventsNotAtVenue
      );
      if (filters.type) {
        events = events.filter(
          (event: any) => event.EventTypeCode === filters.type
        );
        footer.push("type");
      }
      if (filters.format) {
        events = events.filter(
          (event: any) => event.PlayFormatCode === filters.format
        );
        footer.push("format");
      }
      events.sort(
        (a: any, b: any) => getDate(a).valueOf() - getDate(b).valueOf()
      );
      events.forEach((event: any) => {
        ...
        if (event.Address.Name !== Address.Name) {
          const eventUrl = encodeURI(
            `${StoreLocator.wizardsStoreUrl}p=${event.Address.City},+${event.Address.CountryName}&c=${event.Address.Latitude},` +
        ...
        fields.push({
          name:
            ...
            "/" +
            ("0" + eventDate.getDate()).substr(-2),
          value:
            event.Name +
            "\n" +
            `**Format:** ${
              StoreLocator.eventFormatDictionary[event.PlayFormatCode]
            }` +
            (location ? `\n**Location:** ${location}` : "") +
    ...
    return new EmbedBuilder({
      ...
      footer: footer.length
        ? { text: "Events have been filtered by: " + footer.join(", ") }
        : undefined,
      color: 0x00ff00,
    });
  }
  ...
  generateStoreDescription(Address: any, Organization: any): string {
    ...
    const eventUrl = encodeURI(
      `${StoreLocator.wizardsStoreUrl}p=${Address.City},+${Address.CountryName}&c=${Address.Latitude},` +
  ...
  async slashEvents(
    @SlashOption({
      name: "location",
      description: "The name of a location, e.g. New York",
    })
    location: string,
    ...
    if (this.storeSearch) {
      await interaction.reply({
        embeds: [await this.getEvents(location)],
      });
    } else {
      await interaction.reply({
        embeds: [
          await this.generateErrorEmbed(
            null,
            "The list of stores is currently reloading, please wait a few more minutes."
          ),
        ],
      });
    }
