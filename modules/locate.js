    ...
    constructor() {
        ...
        this.commands = {
            locate: {
                ...
                help: 'This command shows you the closest stores with events to a given location.',
    ...
    generateEmbed(results, googleResult) {
        ...
        return rp({url: encodeURI(googleStaticMap.join("")), encoding: null}).then(body =>
            new Discord.RichEmbed({
                title: "Stores for " + googleResult.formatted_address,
                file: {
                    attachment: body,
                    name: "location.png"
                },
        ...
        return rp({url: this.geocoder + encodeURIComponent(location), json: true}).then(googleBody => {
            if (googleBody.results !== null && googleBody.results.length > 0) {
                return rp({
                    ...
                    json: true
                }).then(wizardsBody => this.generateEmbed(wizardsBody.d.Results, googleBody.results[0]),
                    err => this.getErrorEmbed(err, "Couldn't retrieve stores from Wizards."));
            }
        }, err => this.getErrorEmbed(err, "Couldn't retrieve location from Google."));
    }

    getErrorEmbed(err, description) {
        log.error(description, err.error.details);
