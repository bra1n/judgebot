...
import { DApplicationCommand, Discord, Slash, SlashChoice, SlashOption } from "discordx";
...
export default class CR {
    ...
    async cr(
        ...
        // prepare embed
        const embed = new EmbedBuilder().setTitle('Comprehensive Rules').setDescription('Effective ' + this.crData.description).setThumbnail(CR.thumbnail).setURL(CR.location + '/');
        ...
        if (this.crData[rule]) {
            ...
            if (this.crData[rule + ' ex']) {
                embed.setFooter({ text: `Use /${interaction.commandName} examples: True to see examples.` });
            }
            ...
            if (gloss && this.crData[rule[1]]) {
                embed.addFields({ name: 'CR - Rule ' + rule[1], value: this.appendSubrules(rule[1], 1020) });
            }
        }
        else {
